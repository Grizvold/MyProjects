...
static int IsBefore(const void* key1, const void* key2, void *param);
...
int main()
{
    ...
    if(!BSTIterIsSame(BSTFind(new_bst,&arr1[i]), BSTEnd(new_bst)))
    {
        printf("\nproblem finding key that doesnt exist\n");
    }
    for(i = 0 ; i < 23 ; ++i)
    {
